...
-- guard adds it if missing, or raises if it exists as the wrong type.
do $$
...
begin
  ...
  if id_type is null then
    -- No id at all: create it as the PK. Raises if a different PK already
    -- exists, which is the right outcome.
    alter table leads add column id bigint generated by default as identity;
    alter table leads add primary key (id);
  elsif id_type not in ('bigint', 'integer', 'smallint') then
    ...
    raise exception 'leads.id is %, not an integer type; lead_intro_requests.lead_id must match', id_type;
  end if;
end $$;

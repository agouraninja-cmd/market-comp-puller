...
**Measuring a model or prompt change** (2026-08-09, contamination doors closed and a database refusal added 2026-08-10). `run-eval.js` puts the 12 fixed targets in `eval-set.json` through real searches and scores each report with the pure, tested `eval-score.js` (priced sale comps and whether a valuation was possible at all, provenance weighted with `valuation.js`'s own `TIER_WEIGHT`, aggregate-address and out-of-window and off-market rates, narrative lengths against the 2026-08-03 caps, wall clock). It is a SCORECARD, not an assertion suite: nothing has a pass/fail threshold, because a dozen stochastic searches are noisy, and the product is `--compare` between two runs. Summaries land in `docs/evals/`, timestamped in the filename so a same-day, same-label rerun can never silently clobber a possibly-good baseline (committed, so history accumulates); raw reports go to the git-ignored `eval-runs/`. Several things make it trustworthy and must not be undone. The run sends `fresh: true`, an internal-only flag that skips BOTH cache read paths (the exact hit and the derivable-window one), because a cached report would score the model that wrote it and report a false "no difference". Before every run the runner also wipes two local files and records both in the summary: `comp-corpus.jsonl` (`corpusWiped`), because `corpusRowsForMarket` reads that file fallback even with no database configured, so a previous run's harvest would otherwise hand the next run corpus coverage and a smaller search budget; and `subject-sizes.json` (`subjectSizesWiped`), because no eval target supplies `subjectSizeSqft`, so a previous run's building-size lookup would otherwise be found by `findKnownSubjectSize`, again shrinking the search budget and also silently backfilling `subject_size_sqft` regardless of what the run's own model did. The subject-size wipe is not the whole fix: `findKnownSubjectSize` also keeps an in-memory `subjectSizesMem` Map that a file delete cannot clear, so **the server must be restarted, not just have its files wiped, between two runs that are being compared** (the corpus read hits disk on every call with no in-memory layer, so it does not need this). A model comparison already forces a restart because `MODEL` is read once at server startup; this restart rule mainly matters for an A/A run pair meant to measure noise, where nothing else would force one. Before spending anything, the runner also probes the target with `GET /api/stats` (the admin key) and reads `introRequests.db`: a confirmed `true`, a failed probe, or the field simply missing all refuse the run and name the risk, because isolation (`SUPABASE_URL` blank on the server under test) is enforced only by however that server was launched, and a database the runner can see is a database it would both write into and read stale corpus coverage from; only a confirmed `false` proceeds. `EVAL_SKIP_DB_CHECK=1` is the deliberate override for someone who has already verified the database really is disposable. And the runner must target a server started from a separate worktree with `SUPABASE_URL` blank, so every write lands in that worktree's own fallback files instead of production's corpus, market pages, and cache: on Windows this is a documented trap, because in PowerShell `$env:SUPABASE_URL = ""` DELETES the variable rather than emptying it, so server.js's `.env` loader (which only fills vars that are `undefined`) silently restores whatever the worktree's own `.env` holds. Copy ONLY the `ANTHROPIC_API_KEY` line into the eval worktree's `.env`, never the whole file, and prefer a `node -e` launcher that sets `process.env.SUPABASE_URL = ""` (and `SUPABASE_SERVICE_KEY`) explicitly before requiring `./server.js`. A full run costs about $4.30, a model comparison about $8.60. The accuracy backtest (`/api/accuracy`) is the other half of the picture and answers a different question: it scores the reconciliation math over comps already harvested, never what a search found. The **isolation contract itself now lives in `eval-isolation.js`** (2026-08-16), shared with `run-accuracy.js` rather than copied into it: `run-eval.js`'s behavior is byte for byte what it was, and the reason for one copy is that a preflight tightened in one runner and not the other would let the loose one write into production with a slightly better-looking score as the only symptom.

**Measuring whether the comps are TRUE** (2026-08-16; `comp-accuracy.js` + `run-accuracy.js`, spec `docs/superpowers/specs/2026-08-16-comp-search-accuracy-design.md`). Three modules have "accuracy" in their name or their job description and **none of them asks whether a returned comp is true**: `eval-score.js` scores a report's SHAPE (its own spec says every metric is derivable "with no ground truth"), `corpus-audit.js` scores CITATION INTEGRITY and says at length that it must never be read as an accuracy rate, and `backtest.js` scores the reconciliation MATH. Twelve beautifully-tiered comps with the wrong prices score perfectly on all three. `comp-accuracy.js` is the pure, tested module that closes it, in two families that must not be confused when reading the output. **Self-consistency needs no answer key, costs nothing and is deterministic**, so its findings are DEFECTS rather than signals: `psf_mismatch` (a sale whose stated `price_per_sqft` does not match price ÷ size), `alt_basis_mismatch` (the same arithmetic for `price_per_unit` and `price_per_acre`, the headline figure for Multifamily and Land, which nothing else checks), `rate_as_sale_price` (a sale priced under $10k on a building over 1,000 SF, which catches both the "$1.2M" shorthand `numericValue` reads as `1.2` and a lease rate in the price field), `implausible_psf`, `future_date`, and `duplicate_comp` (one building returned twice under two spellings votes twice in every median above it). **Ground-truth agreement** scores recall plus price/size/date fidelity against `truth-set.json`, at 2% / 5% / 31 days — three different tolerances on purpose, since a recorded sale price is exact while gross-vs-rentable size and recording-vs-closing date legitimately differ between sources. Four rules a future editor will otherwise break. **`unverified` is NOT an error rate and nothing may turn it into one**: the answer key holds the deals we happen to know, so a comp absent from it is unproven, never false, and reporting it as a false-positive rate would produce a confident number that mostly measures how small the key is (pinned by a test, and the runner re-states it every time it prints the count). **`basis: model_corpus` is refused by the validator by name** — a deal a past search produced is not independent of the thing being measured, the same rule `backtest.js`'s `GROUND_TRUTH_TIERS` carries. **Every borrowed rule is required, never restated** (`normAddress` from `backtest.js`, `cityStateOf` from `eval-score.js`, `numericValue` from `valuation.js`, `isAggregateAddress` from `corpus-audit.js`), and the module deliberately overlaps none of the three above. **The committed `truth-set.json` is validated by `npm test`**, so a malformed answer key fails the build instead of silently scoring nothing. `run-accuracy.js` has four modes and only one spends money: `--validate`, `--corpus` (free, family 1 over every harvested comp, the mode that produces a real number on day one with an empty key), `--offline <dir>` (free, over reports already on disk including `eval-runs/`), `--seed-from-submissions` (merges APPROVED broker submissions in as `broker_verified`, carrying no broker name, email, phone or company, because this repo is public and the vault is never a source), and the default billed run, one search per market+type the key holds. Summaries land in `docs/accuracy/`; raw reports go to the git-ignored `accuracy-runs/`. Nothing here ships to a marketing surface, same standing rule as `/api/accuracy` and the corpus audit.

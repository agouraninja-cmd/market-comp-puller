...
async function runPage(comps, benchResult, opts, identity) {
  ...
  const fakeFetch = (url, init) => {
    ...
    if (u.indexOf("/api/vault/inspect") === 0) {
      ...
      const info = VAULT.inspectCsv(String(req.csv || ""));
      ...
      return Promise.resolve(jsonResponse(200, Object.assign({}, info, {
        ...
        marketSuggest: marketSuggestOf(VAULT.csvAddresses(String(req.csv || ""),
          { mapping: req.mapping || null }), opts),
...
// ---------------------------------------------------------------------------
// The lease side of the book (migration 029)
//
...
test("picking a market stamps the address cells, cures the market-only rows, and posts the completed rows", async () => {
  ...
  const cells = addressCells(doc);
  assert.equal(cells[0].getAttribute("data-raw"), "4100 W Franklin Rd, Boise, ID",
    "the stamp is visible in the row's own address cell");
  assert.equal(cells[1].getAttribute("data-raw"), "88 Freight Ln, Boise, ID");
  assert.equal(cells[2].getAttribute("data-raw"), "1 Ready Way, Boise, ID", "a whole address is untouched");
...
test("a hand-typed address is never overwritten, a re-pick corrects only the stamped ones, and blank puts them back", async () => {
  ...
  const sel = doc.getElementById("pdfMarketPick");
  sel.value = "Boise, ID"; sel.fire("change");
  assert.equal(addressCells(doc)[0].getAttribute("data-raw"), "4100 W Franklin Rd, Boise, ID");
  assert.equal(addressCells(doc)[1].getAttribute("data-raw"), "88 Freight Ln, Caldwell, ID",
  ...
  assert.equal(addressCells(doc)[0].getAttribute("data-raw"), "4100 W Franklin Rd, Nampa, ID",
    "a stamped row follows a corrected pick, recomposed from the bare street");

  sel.value = ""; sel.fire("change");
  assert.equal(addressCells(doc)[0].getAttribute("data-raw"), "4100 W Franklin Rd", "blank puts it back");

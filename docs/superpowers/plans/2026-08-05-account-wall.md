...
Two tests fail before you change anything: `market explorer guest cap` and `market explorer with the guest gate disabled`. They belong to another session's in-flight Market Explorer work, and they also trip over a git-ignored `market-pages-dynamic.json` that local testing leaves behind. **Do not fix them and do not delete that file.** Your bar is: no *new* failures, and every test you add passes.

**2. This checkout is shared.** A second Claude session and a human collaborator write to the same working tree. `test/routes.test.js` currently has uncommitted changes that are not yours. Before every `git add`: run `git status --short`, stage explicit paths only, never `git add -A`. Task 2 touches that contended file and carries its own procedure.

...
| 017-broker-property-coordinates.sql | **NOT RUN — pending** | Written 2026-08-06 alongside the code that uses it. **Deploy-safe to leave pending:** both readers swallow their own errors by design — `attachPropertyCoords()` returns the comps unchanged and `linkVaultProperties()` logs and gives up — so an un-migrated database degrades to exactly today's behaviour (private comps geocoded by address) rather than to an error, and no upload fails. That is also why it is easy to forget: `node migrations/verify.js` is the thing that will notice, and it does. Run the whole file in the SQL editor; it is wrapped in begin/commit and every statement is `if not exists`, so re-running is a no-op. The three CHECK constraints are safe to add to the live table because every existing row has both coordinate columns null. |

...
from the same rule the server fills with) and **`firm-skyline.js`** (the Workspace
banner drawn as the firm's skyline — what each tower's height, lit windows,
red light and crane mean — dual-exported, since the page draws what it decides) — plus **`report-access.js`** (the ONLY function that
